...
namespace njin::gltf {
        ...
        elements_{ [&]() -> std::vector<Element> {
            ...
            while (current_element < info.count) {
                if (info.type == Type::Vec3 &&
                    ...
                    math::njVec4f element{ component_1,
                                           component_2,
                                           component_3,
                                           component_4 };
                    elements.push_back(element);
    ...
    std::vector<math::njVec4f> Accessor::get_vec4f() const {
        std::vector<math::njVec4f> elements{};
        for (const auto& element : elements_) {
            elements.push_back(std::get<math::njVec4f>(element));
        }
