...
namespace {
    VkDeviceCreateInfo make_device_create_info(
     ...
     */
    std::vector<VkDeviceQueueCreateInfo> make_queue_create_infos(
    ...
    make_queue_create_info(uint32_t queue_family_index, const float* priority) {
...
namespace njin::vulkan {
    LogicalDevice::LogicalDevice(const PhysicalDevice& physical_device) {
        if (!check_physical_device_extension_support(physical_device,
            ...
            throw
            std::runtime_error("Requested device extensions not supported by physical device");
        }
